...
class Cliente extends Spine.Model
  @configure 'Cliente', 'Documentos' ,'Name', 'Codigo' , 'Saldo' , 'Credito'  , 'Meta' , 'Venta' ,'SubRuta' , 'Vendedor' , 'DiasCredito__c' , "Activo" , "Telefono" 
  ...
  @fetch_from_sf: (user) ->
    query = "Select Id , Name, DiasCredito__c , Meta__c, Vendedor__r.VendedorId__c , 
    CodigoExterno__c , Saldo__c, CreditoAsignado__c  , Activo__c , Telefono__c from Cliente__c where Saldo__c != 0"
  ...
  @set_current: (cliente,lock = false) =>
    result = false
